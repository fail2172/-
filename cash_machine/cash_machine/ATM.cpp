...
void ATM::insert_card(Card card)
{
    for (size_t i = 0; i < database.size(); ++i) {
        if (card.bank_name == database[i].bank_name &&
            card.cardholder_name == database[i].cardholder_name &&
            card.number_card == database[i].number_card &&
            card.validity == database[i].validity) {
                sys_num_card = i;
                break;
        }
    }
...
ATMsession::ATMsession(ATM curr_atm)
{
    if (curr_atm.current_card.number_card == "")throw 5;

}

void ATMsession::get_pincode()
{
...
void Options::processing(ATM& session)
{
    char chose;
    do {
        try {
...
void Сheck::print_a_check(ATM& session)
{
    if (display()) {
...
void Options::withdraw_funds(ATM& session)
{
    int sum;
...
void Options::withdraw_funds_for_phone(ATM& session)
{
...
void Options::withdraw_funds_for_any_card(ATM& session)
{
